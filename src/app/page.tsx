  ...
  const analyzeResume = async () => {
    ...
    try {
      const response = await fetch(
        "https://YOUR-RENDER-LINK.onrender.com/analyze",
        {
          method: "POST",
          body: formData,
        }
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <nav className="bg-blue-600 text-white p-4 flex justify-between">
        <h1 className="text-xl font-bold">ATS Optimizer</h1>
      ...
      <div className="flex items-center justify-center mt-10">
        <div className="bg-white p-10 rounded-2xl shadow-xl w-[500px] text-center">
          ...
          {fileName && (
            <p className="mt-4 text-green-600">Uploaded: {fileName}</p>
          )}
          ...
          <div className="mt-6 flex justify-center gap-4">
            <button
              ...
              className={`px-6 py-3 rounded-xl text-white ${
                fileName === "" ? "bg-gray-400" : "bg-blue-600"
              }`}
            >
              Analyze Resume
            </button>
          ...
          {loading && (
            <p className="mt-4 text-blue-600">Analyzing Resume...</p>
          )}
          ...
          {score !== null && (
            <div className={`mt-6 p-4 rounded-xl ${getScoreColor()}`}>
              ...
              <p className="mt-2 text-lg text-gray-700">{getStatus()}</p>
            </div>
          )}
          ...
          {matchedKeywords.length > 0 && (
            <div className="mt-6 text-left">
              <h3 className="text-xl font-bold mb-2">Matched Keywords</h3>
          ...
          <div className="mt-6 text-left">
            <h3 className="text-xl font-bold mb-2">Resume Tips</h3>
          ...
          <p className="mt-8 text-sm text-gray-500">
            Built using Next.js, React, Tailwind CSS and Python Flask
          </p>
        </div>
      </div>
    </div>
